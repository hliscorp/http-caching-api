...
class CacheValidator
{
     ...
     */
    public function __construct(CacheRequest $request): void
    {
     ...
     * @return boolean
     */
    private function isFreshEnough(int $age, int $freshness): bool
    {
        if ($age < $freshness) {
            return true;
        }
     ...
     * @return boolean
     */
    private function allConditionalsMatch(Cacheable $cacheable): bool
    {
        ...
        if ($noneMatch && $modifiedSince && !($etagValidatorMatches && $lastModifiedValidatorMatches)) {
            return false;
        } elseif ($noneMatch && !$etagValidatorMatches) {
            return false;
        } elseif ($modifiedSince && !$lastModifiedValidatorMatches) {
            return false;
        } else {
            return true;
        }
